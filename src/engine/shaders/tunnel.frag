...
uniform float uScroll;       // integrated forward travel (CPU-accumulated so speed changes never jump)
...
void main() {
  ...
  // Organic distortion (uTunnelShape: 0 = geometric, 1 = vulval)
  // Mirror-symmetric (left/right) folds, vertical slit opening, hood at
  // the top, inner lips rippling with the peristaltic wave, breath-driven
  // opening. Larger r = wall closer to the viewer.
  if (uTunnelShape > 0.01) {
    float org = uTunnelShape;
    float t   = uTime;
    float cA  = cos(angle);          // +1 right, -1 left
    float sA  = sin(angle);          // +1 top, -1 bottom
    float c2  = cA * cA;             // 1 at sides, 0 at top/bottom

    // Vertical slit: walls press in from the sides, open along the vertical
    r *= 1.0 + c2 * 0.5 * org;

    // Outer labia: broad symmetric folds, slowly rolling
    r += (cos(angle * 2.0) * 0.045
        + cos(angle * 4.0 + sin(t * 0.17) * 0.6) * 0.02) * org;

    // Inner labia: finer ripples riding the peristaltic wave
    float wave = sin(r * 11.0 - t * 0.9) * 0.5 + 0.5;
    r += (cos(angle * 2.0) * 0.5 + 0.5) * wave * 0.035 * org
       + (cos(angle * 6.0 - t * 0.25) * 0.5 + 0.5) * wave * 0.01 * org;

    // Hood: a soft bump at the top of the slit
    float hood = pow(max(sA, 0.0), 6.0) * (1.0 - c2);
    r += hood * 0.09 * org;

    // Peristalsis along depth
    r += (sin(r * 15.0 - t * 1.5) * 0.02
        + sin(r * 8.0  + t * 0.8) * 0.015) * org;

    // Tiny asymmetry so it never looks stamped
    r += uvAspect.x * sin(t * 0.1) * 0.012 * org
       + uvAspect.y * cos(t * 0.13) * 0.01 * org;

    // Breath: opens on inhale, closes on exhale; warm throb under it
    r *= 1.0 - br * 0.07 * org;
    r *= 1.0 - (sin(t * 0.7) * 0.5 + 0.5) * 0.02 * org;
  }
  ...
  float ringFreq = 8.0; // fixed: easing a frequency makes rings slide
  ...
  float segCount = 3.0; // fixed: easing an angular frequency makes ribs spin
  ...
  cylLight = mix(cylLight, 0.78 + 0.22 * cos(angle - PI * 0.5), uTunnelShape);
  ...
  // Moist sheen: glossy highlights along ridge edges in organic mode
  wall += uColor3 * ringEdge * ringEdge * uTunnelShape * 0.35 * depthLight;
